...
public class MatriceDynamiqueNonCarre {
	...
	public static void main(String[] args) {
		...
		do
		{
		  ...
		  int m[][]= new int [taille][]; //matrice non carrée
	for  (int i=0; i<m.length; i++)
	{
		int v; //taille de chacun des tableaux
		do
		{
			System.out.print("Saisir la taille du tableau numéro : "+(i+1)+ " ");
			 v=in.nextInt();
		} while (v<=0);
		m[i]=new  int[v]; //alocation du tableau numéro i+1
		for (int j=0; j<v; j++)
		{
			System.out.print("Saisir l'élement à la position ("+i+","+j+")");
			m[i][j]=in.nextInt();
		}
	}
